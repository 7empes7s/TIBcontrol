...
function scanRootPlanFiles(): BuilderPlanCandidate[] {
  ...
  // Scan /root for top-level plan/roadmap/agent docs
  try {
  ...
  // Scan /root/.claude/plans/ — operator-curated plan files created by Claude
  const claudePlansDir = "/root/.claude/plans";
  try {
    if (existsSync(claudePlansDir)) {
      for (const entry of readdirSync(claudePlansDir, { withFileTypes: true })) {
        if (!entry.isFile() || !entry.name.endsWith(".md")) continue;
        const path = join(claudePlansDir, entry.name);
        candidates.push({
          path,
          title: readTitle(path),
          kind: "project",
          exists: true,
          modifiedAt: fileModifiedAt(path),
          relevance: "Operator plan file from ~/.claude/plans/ — curated by Claude for this project.",
        });
      }
    }
  } catch {}

  // Scan /home/agent/.claude/plans/ if present
  const agentClausePlansDir = "/home/agent/.claude/plans";
  try {
    if (existsSync(agentClausePlansDir)) {
      for (const entry of readdirSync(agentClausePlansDir, { withFileTypes: true })) {
        if (!entry.isFile() || !entry.name.endsWith(".md")) continue;
        const path = join(agentClausePlansDir, entry.name);
        candidates.push({
          path,
          title: readTitle(path),
          kind: "project",
          exists: true,
          modifiedAt: fileModifiedAt(path),
          relevance: "Agent plan file from ~/.claude/plans/ — curated for this project.",
        });
      }
    }
  } catch {}
...
export function getBuilderModelsInventory(): BuilderModelsInventory {
  ...
  const opencodeFallbacks = opencodeModels
    .filter((model) =>
      model.capability === "heavy" ||
      model.name.includes("minimax") ||
      model.name.includes("kimi") ||
      model.name.includes("codex")
    )
    .slice(0, 8)
