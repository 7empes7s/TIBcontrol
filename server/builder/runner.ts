...
function buildContinuationContext(workflow: BuilderWorkflow, run: BuilderRun, nextSequence: number, agent?: string, model?: string | null): string {
  ...
  for (const pass of prevPasses.sort((a, b) => a.sequence - b.sequence)) {
    ...
    const timedOut = pass.failureClass === "timeout";
    const passHeader = timedOut
      ? `--- Pass ${pass.sequence} (${pass.agent ?? "?"}, TIMED OUT — pick up mid-task) ---`
      : `--- Pass ${pass.sequence} (${pass.agent ?? "?"}, ${pass.status}) ---`;
    lines.push(passHeader);
    if (timedOut) {
      lines.push("IMPORTANT: the previous pass was cut off by a timeout. Resume exactly where it stopped.");
      lines.push("Do NOT restart from the beginning. Check the last stdout lines below to find the last completed step.");
    }
    if (pass.summary) {
...
async function startNextPass(
  ...
  let { model, provider, reason: modelReason } = selection;

  // When the scheduled agent (gemini, codex, claude) has no compatible model available
  // in the fallbackTargets, degrade to the opencode agent which can use any model.
  // This ensures minimax/kimi fallbacks actually trigger instead of running with no model.
  if (!model && agent !== "opencode") {
    const degraded = selectModelForRole(role, workflow.config, "opencode");
    if (degraded.model) {
      console.log(`[builder] ${agent} has no compatible model for pass ${nextSequence}; degrading to opencode/${degraded.model} (${degraded.reason})`);
      agent = "opencode";
      model = degraded.model;
      provider = "opencode";
      modelReason = `agent-degraded:${entry.agent}->${degraded.reason}`;
    }
  }
  ...
  // Use systemd-run --scope to start the tmux session outside the service's cgroup.
  // Without this, a `systemctl restart control-surface.service` call (from the agent
  // or the operator) kills all builder tmux sessions since they share the service cgroup.
  const spawnResult = spawnSync(
    "systemd-run", ["--scope", "--collect", "--quiet", "--",
      "tmux", "new-session", "-d", "-s", session, "-c", workflow.projectRoot, scriptPath],
    { encoding: "utf8" },
  );
...
DO NOT run: systemctl restart control-surface.service (or stop/start it).
Reason: the builder runs INSIDE that service's cgroup — restarting it kills
this pass mid-work. The reconciler automatically builds and restarts the
service after the pass exits cleanly. Your job is code edits + typecheck only.
=============================================================================`;
...
function writePassScript(
  ...
  const PASS_TIMEOUT_SECONDS = workflow.config.riskPolicy?.passTimeoutSeconds ?? 7200;
...
# Intercept systemctl restart/stop control-surface.service — restarting the service
# kills this very tmux session (same cgroup). The reconciler restarts the service
# after the pass finishes. Silently skip the call and log it instead.
systemctl() {
  local _action="\${1:-}" _unit="\${2:-}"
  if [[ "$_unit" == *"control-surface"* ]] && [[ "$_action" =~ ^(restart|stop|start)$ ]]; then
    echo "[builder] Skipping systemctl $* — control-surface restarts happen after the pass exits" >> "$BUILDER_DIR/${stdoutLog}"
    return 0
  fi
  command systemctl "$@"
}
...
export async function startWorkflowRun(
  ...
  // Spawn tmux outside the service cgroup so it survives service restarts.
...
// In-memory lock: prevents concurrent reconciler ticks from processing the same
// run simultaneously. When one tick is running validations (which are async and
// take up to ~60s), subsequent ticks must not start a duplicate validation pass.
const activeReconciliations = new Set<string>();

export async function reconcileRunStatus(runId: string): Promise<BuilderRun | null> {
  if (activeReconciliations.has(runId)) return readBuilderRun(runId);
  const run = readBuilderRun(runId);
  ...
  if (tmuxExists(session)) {
    ...
    const STALL_THRESHOLD_MS = (workflow?.config.riskPolicy?.stallTimeoutSeconds ?? 600) * 1000;
  ...
  // Claim exclusive reconciliation for this run (second guard after the in-memory Set).
  // The Set check above handles concurrent async ticks; this DB re-read handles restarts.
  activeReconciliations.add(runId);
  try {
  ...
  // A timed-out pass (exit 124) or SIGTERM'd pass (exit 143, e.g. from OOM pressure or
  // memory cascade) can still produce valid changes. If all validations pass we treat the
  // run as successful — the agent completed its useful work before being killed.
  const passTimedOut = exitCode === 124;
  const passSigterm = exitCode === 143;
  let runStatus: "success" | "failed" = "failed";
  if (exitCode === 0 || passTimedOut || passSigterm) {
  ...
  } else if ((passTimedOut || passSigterm) && runStatus === "success") {
    runError = null; // killed mid-run but changes validated — not an error
  } else if (passTimedOut) {
    runError = "Pass timed out and validations did not all pass";
  } else if (passSigterm) {
    runError = "Pass SIGTERM'd and validations did not all pass";
  } else if (exitCode !== 0) {
  ...
  // Check if we should continue to next pass before updating run status.
  // Must happen BEFORE updateBuilderRun so we keep the run "running" while more passes remain.
  const AUTO_CONTINUE_MODES = new Set(["auto-continue", "scheduled", "permanent"]);
  const continueOnTimeout = Boolean(workflow?.config.riskPolicy?.continueOnTimeout);
  const shouldContinue = Boolean(
    workflow &&
    passSeq < workflow.config.riskPolicy.maxPasses &&
    (
      // Normal: successful pass in an auto-continue mode workflow
      (runStatus === "success" && AUTO_CONTINUE_MODES.has(workflow.mode)) ||
      // Timeout continuation: timed-out pass with the flag set (any mode)
      (passTimedOut && continueOnTimeout)
    )
  );

  // When continuing, keep the run "running" so the next pass can be reconciled.
  // Only finalize (set finishedAt + final status) on the last pass.
  updateBuilderRun(runId, {
    status: shouldContinue ? "running" : runStatus,
    finishedAt: shouldContinue ? undefined : ts,
    currentPassId: null,
    error: shouldContinue ? null : runError,
  });
  ...
  `).run(
    shouldContinue ? "running" : runStatus,
    shouldContinue ? "running" : runStatus,
    shouldContinue ? null : ts,
    stdout ? stdout.slice(-8000) : null,
    shouldContinue ? null : runError,
    exitCode,
    "builder-run",
    runId,
    "running",
  ...
  if (workflow) {
    if (!shouldContinue) {
      ...
      // After a successful control-surface pass, schedule a deferred service restart
      // so the new build is served. Deferred to avoid killing this reconciler tick.
      if (runStatus === "success" && workflow.projectRoot === "/opt/opencode-control-surface") {
        setTimeout(() => {
          try {
            spawnSync("systemctl", ["restart", "control-surface.service"], { encoding: "utf8" });
            console.log("[builder] control-surface.service restarted after successful pass");
          } catch (e) {
            console.error("[builder] failed to restart control-surface.service:", e);
          }
        }, 5000);
      }
...
async function runRuntimeValidation(
  ...
  if (internalUrl) {
    ...
    let error: string | null = null;
    let status = "failed";
    // Use async fetch so the server's own event loop can process the incoming request.
    // spawnSync("curl") blocks the event loop and causes the server to miss its own
    // smoke-check request, resulting in HTTP 000 even when the service is healthy.
    const SMOKE_RETRIES = 5;
    const SMOKE_BACKOFF_MS = 3000;
    for (let attempt = 0; attempt < SMOKE_RETRIES; attempt++) {
      try {
        const resp = await fetch(internalUrl, { signal: AbortSignal.timeout(10_000) });
      ...
      if (status !== "success" && attempt < SMOKE_RETRIES - 1) {
        await new Promise<void>((resolve) => setTimeout(resolve, SMOKE_BACKOFF_MS));
      }
    }
...
async function runPublicValidation(
  ...
  if (publicUrl) {
    ...
    for (let attempt = 0; attempt < SMOKE_RETRIES; attempt++) {
      try {
        const resp = await fetch(publicUrl, { signal: AbortSignal.timeout(10_000) });
        if (resp.status >= 200 && resp.status < 400) {
          status = "success";
          error = null;
          break;
...
async function runPlaywrightValidation(
  ...
  for (const target of targets) {
    ...
    const pwScript = `
import { chromium } from '/root/.claude/skills/dashboard-orchestrator/node_modules/playwright/index.mjs';
...
try {
  ...
  console.log(JSON.stringify({ ok: false, error: e.message }));
    ...
    try {
      // Use async spawn so the server's event loop stays free to respond to the
      // Playwright browser's HTTP requests. spawnSync blocks the loop and causes
      // the browser to time out waiting for the very server running this code.
      const { stdout, exitCode } = await new Promise<{ stdout: string; exitCode: number }>((resolve) => {
        const chunks: Buffer[] = [];
        const errChunks: Buffer[] = [];
        const child = spawn("node", [scriptPath], {
          cwd: projectRoot,
          env: { ...process.env, NODE_PATH: "/root/.claude/skills/dashboard-orchestrator/node_modules" },
          stdio: ["ignore", "pipe", "pipe"],
        });
        child.stdout?.on("data", (d: Buffer) => chunks.push(d));
        child.stderr?.on("data", (d: Buffer) => errChunks.push(d));
        const timer = setTimeout(() => { child.kill("SIGTERM"); resolve({ stdout: "", exitCode: 28 }); }, 45_000);
      ...
      try {
        ...
        else error = `playwright error: ${exitCode}`;
      }
    } catch (e) {
      error = e instanceof Error ? e.message : String(e);
      status = "error";
