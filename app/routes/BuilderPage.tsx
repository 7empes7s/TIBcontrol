      ...
      <div className="modal-box builder-workflow-modal structured" onClick={(event) => event.stopPropagation()}>
        ...
        <div className="builder-form-grid">
          ...
          <ButtonGroup
            ...
            onChange={(v) => setDraft((prev) => ({ ...prev, planFile: v as string }))}
            options={selectablePlans.map((plan) => ({
              value: plan.path,
              label: plan.exists ? plan.title : `[create] ${plan.title}`,
              disabled: !plan.exists && draft.mode !== "plan",
            }))}
          />
          <ButtonGroup
            label="Mode"
            ...
            <>
              ...
              <ButtonGroup
                  ...
                  onChange={(v) => setDraft((prev) => ({
                    ...prev,
                    config: { ...prev.config, schedule: { ...prev.config.schedule, timezone: v as string } },
                  }))}
                  options={[
                    { value: "UTC", label: "UTC" },
...
function CollapsibleSection({ title, children, defaultOpen = false }: { title: string; children: React.ReactNode; defaultOpen?: boolean }) {
  ...
  return (
    <div className={`section-card${open ? "" : " closed"}`}>
      <div className="section-card-header sc-header" onClick={() => setOpen(!open)}>
      ...
      {open && <div className="section-card-body" style={{ padding: "12px 14px" }}>{children}</div>}
    </div>
  );
}
...
export function BuilderPage() {
  ...
  const workflowsPage = useTablePage(workflows);
  const runsPage = useTablePage(runs);
      ...
      {data && (
        <>
          ...
          <SectionCard
            ...
            right={
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <button className="btn btn-xs btn-ghost" onClick={() => setShowProvision(true)}>
                  <Plus size={12} /> bootstrap
                </button>
                <span className="mono dim">{workflows.length} saved</span>
              </div>
            }
          >
            <div className="section-card-body table-wrap">
              {workflowsApi.data?.degraded ? (
                ...
                <table className="data-table workflows-table">
                  ...
                  <tbody>
                    {workflowsPage.slice.map((workflow) => {
            ...
            <TablePageControls {...workflowsPage} onPrev={workflowsPage.prev} onNext={workflowsPage.next} onSetPageSize={workflowsPage.setPageSize} noun="workflows" />
          </SectionCard>
          ...
          >
            <div className="section-card-body table-wrap">
              {runsApi.data?.degraded ? (
                ...
                <table className="data-table runs-table">
                  ...
                  <tbody>
                    {runsPage.slice.map((run: BuilderRun) => (
            ...
            <TablePageControls {...runsPage} onPrev={runsPage.prev} onNext={runsPage.next} onSetPageSize={runsPage.setPageSize} noun="runs" />
          </SectionCard>
