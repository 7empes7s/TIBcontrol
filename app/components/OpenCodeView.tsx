...
import { AgentModelPicker, type AgentModel } from "./AgentModelPicker";
...
export function OpenCodeView() {
  const {
    ...
    currentModel, providers, loadProviders, abortSession, setModel,
  } = useStore();
  ...
  const allModels = useMemo<AgentModel[]>(() => {
      ...
      <main className="oc-main">
        ...
        {activeSession && (
          <AgentRuntimeBar
            ...
            models={allModels.map(m => ({ name: m.id, label: m.name }))}
            approvalMode="default"
            disabledFeatures={["approvalMode"]}
          />
        )}
        {activeSession && (
          <AgentComposer
